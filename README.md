...
| 屏蔽国家名单 | JSON 数组，存储国家缩写（如 `["CN","RU"]`），被屏蔽的国家无论成功率和价格如何都不会被选中 |

### 5.4 号池监控

管理后台新增「号池监控」页面，用于实时查看指定服务的 SMS 号池情况，辅助判断号源健康度和策略合理性。

**操作方式：**

- 顶部选择 Provider 和 Service，点击「查询」
- 刷新：手动重新点击查询获取最新数据
...
| 指标 | 说明 |
|------|------|
| 当前策略 | 当前 Service 配置的成功率阈值和最高单价 |
| 可用国家 | SMS 平台返回的该服务全部可用国家数 |
| 符合策略 | 满足成功率阈值、价格上限且不在屏蔽名单中的国家数 |
| 已屏蔽 | 被屏蔽名单排除的国家数 |
| 优选国家（前3） | 符合策略中按价格排序的前 3 个国家，即实际下单优先尝试的顺序 |
...
| 成功率 | 带进度条可视化，低于阈值时标红 |
| 最低价格 | 该国当前最低号码单价 |
| 库存 | 当前可用号码数量；为 0 时标红，小于 10 时标黄 |
| 策略状态 | 符合策略（绿色）/ 不符合并注明原因 / 已屏蔽（橙色）|

**Tab 过滤：**

| Tab | 说明 |
|-----|------|
| 全部 | 所有国家，排序：符合策略 → 不符合 → 已屏蔽 |
| 符合策略 | 仅显示通过策略筛选的国家，按价格升序 |
| 已屏蔽 | 仅显示被屏蔽名单排除的国家 |

**可扩展性：** 接入新 SMS 平台时只需在对应 adapter 实现 `getPoolStatus()` 方法，号池监控页面无需改动，Provider 下拉自动新增。
...
1. 获取该服务下所有可用国家及其成功率、价格和库存
2. 排除在该 Service **屏蔽国家名单**中的国家
3. 过滤掉成功率低于该 Service 配置阈值的国家
4. 过滤掉单价超过该 Service 最高单价的国家
5. 在剩余国家中按**价格升序**排列，依次尝试前 3 个国家下单，返回第一个成功的结果
6. 若无任何符合条件的国家（屏蔽后），自动 fallback 到全部未屏蔽国家中成功率最高的前 3 个
...
SmsProvider Interface
  ├── orderNumber(serviceId, options)   取号（含屏蔽名单、成功率、价格策略）
  ├── pollOrder(orderId)                轮询短信状态
  ├── cancelOrder(orderId)              取消订单
  └── getPoolStatus(serviceId)          查询号池状态（各国成功率/价格/库存）

SmsPoolAdapter   implements SmsProvider   ← 当前实现
FutureAdapter    implements SmsProvider   ← 未来扩展
```

新增 SMS 平台时，只需新增一个适配器 + 配置环境变量，原有 CDK/Order 逻辑和号池监控页面均无需改动。
