...
> 本文档记录各 CDK 类型的需求讨论结果及待确认事项。
> 当前阶段：**短效时效型（Type A）已可进入实现；号码绑定型（Type C）需求已确认，可进入实现；长效 PingMe 型（Type B）待续**。
...
| 新 C | **号码绑定型**     | 时间（固定次日 07:00 到期）     | yamasakisms        | 需求已确认，可进入实现 |
...
## 四、号码绑定型（Type C）——需求已确认

详细规格见 [CDK-TYPE-BOUND-SPEC.md](./CDK-TYPE-BOUND-SPEC.md)。

### 核心规则摘要

- CDK 取号即绑定，**取号即消耗**（不等接到验证码）
- 有效期固定到**次日 07:00（北京时间 UTC+8）**，不可配置
- **08:00 前不开放取号**（距下一个 07:00 不足 1 小时，无意义）
- 一个手机号只能绑定 1 个 CDK（唯一约束）
- 用户不可换号，只有管理员可操作换号
- 对外暴露接码 API：`GET /api/{CDK}`，不透出第三方平台信息
- 验证码来源：yamasakisms，通过 `order_no` 调 `getphonecode`
...
号码绑定型  → 取号时立刻 exhausted；order 到次日 07:00 → expired
```
...
2. **实现号码绑定型（Type C）**
   - 详见 CDK-TYPE-BOUND-SPEC.md

3. **后续实现长效 PingMe 型（Type B）**
   - 完成上方待确认问题
   - 新增 PingMe 适配器
   - 新增 Webhook 接收端点
   - 新增独立前端页面
