...
export default function BoundCdk() {
  ...
  const doValidate = useCallback(async (code: string) => {
    ...
    try {
      ...
      const data = (await res.json()) as BoundValidateResult & {
        error?: string;
        expiredAt?: string;
        boundAt?: string;
      };
  ...
  // 从 Home 跳转过来时携带 code 参数，自动发起校验
  useEffect(() => {
    const code = searchParams.get("code");
    if (code) {
      doValidate(code.toUpperCase());
    } else {
      inputRef.current?.focus();
    }
  }, [searchParams, doValidate]);

  // ── 校验 CDK ──────────────────────────────────────────────────────────────

  function handleValidate() {
    doValidate(inputCode.trim().toUpperCase());
  }
  ...
  async function handleTakeNumber() {
    ...
    try {
      ...
      if (!res.ok) {
        setPageState({
          kind: "error",
          message: data.error ?? `取号失败 (${res.status})`,
        });
        return;
      }
      ...
      setPageState({
        kind: "error",
        message: err instanceof Error ? err.message : "取号失败，请重试",
      });
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">
                CDK 有效，点击"立刻取号"获取专属号码
              </p>
              <p className="text-xs text-gray-400 mt-1">
                取号后 CDK 立即消耗，有效期至次日 07:00
              </p>
              <p className="font-mono text-xs text-gray-500 mt-2 bg-gray-50 px-3 py-1.5 rounded-lg">
                {pageState.code}
              </p>
            </div>
            ...
            <div className="border-t border-gray-100 pt-4 space-y-1.5 text-sm">
              <div className="flex justify-between text-gray-600">
                ...
                <span className="font-medium text-gray-800">
                  {calcRemainingHours(pageState.order.expiresAt)}
                </span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>将于</span>
                <span className="font-medium text-gray-800">
                  {formatDateTime(pageState.order.expiresAt)} 失效
                </span>
              </div>
              {pageState.order.boundAt && (
                <div className="flex justify-between text-gray-600">
                  <span>已于</span>
                  <span className="font-medium text-gray-800">
                    {formatDateTime(pageState.order.boundAt)} 取号
                  </span>
                </div>
              )}
            </div>
        ...
        {pageState.kind === "expired" && (
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-5">
            ...
            <div className="space-y-2 text-sm">
              {pageState.expiredAt && (
                <div className="flex justify-between text-gray-600">
                  <span>过期时间</span>
                  <span className="font-medium text-gray-800">
                    {formatDateTime(pageState.expiredAt)}
                  </span>
                </div>
              )}
              {pageState.boundAt && (
                <div className="flex justify-between text-gray-600">
                  <span>最后取号时间</span>
                  <span className="font-medium text-gray-800">
                    {formatDateTime(pageState.boundAt)}
                  </span>
                </div>
              )}
            </div>
            ...
            <p className="text-sm text-gray-500 bg-gray-50 rounded-xl px-4 py-3">
              该号码已过期，无法继续接收验证码。如需继续使用，请联系管理员获取新的
              CDK。
            </p>
