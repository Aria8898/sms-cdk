...
| 层 | 改动 |
|----|------|
| DB | `cdks` 表新增 `cdk_type TEXT NOT NULL DEFAULT 'count'`（枚举：`count` / `timed`）、`validity_minutes INTEGER`；`orders` 表新增 `error_message TEXT`、`cost DECIMAL`、`ip_address TEXT`、`completed_at TIMESTAMP`；新增 `login_attempts` 表（id / ip_address / success / created_at） |
| 后端 | `validate`：检测进行中订单返回 `activeOrder`；timed CDK 过期时错误响应附带 `expiresAt` + `lastOrderedAt`；**IP 限频 10 次/分钟（`RATE_LIMIT_VALIDATE` env var），超限返回 429** |
| 后端 | `order`：timed 类型 `expiresAt = now + validity_minutes × 60s`；写入 `ip_address`；终态时写入 `completed_at` |
| 后端 | `order/:id/change`：timed 类型换号时同步重置 `expiresAt` |
| 后端 | `order/:id/status`：timed 类型 `canRetry = expiresAt > now`；`expired` 路径区分两种情况（见下）；终态时写入 `completed_at` |
| 后端 | `auth/login`：**IP 限频 5 次/分钟，连续失败超限封禁 15 分钟，返回 429**；每次尝试写入 `login_attempts`；新增 `NOTIFY_WEBHOOK_URL` env var 预留（空则不通知） |
...
| Admin | 新增安全日志页：展示登录尝试记录（IP / 成功失败 / 时间），支持按失败筛选 |
...
**可见效果**
Admin 可生成时效型 CDK，填入有效分钟数；用户兑换时倒计时根据 CDK 配置显示；有效期内无限再发一条不扣次；用户中途离开页面，有效期内重新输入 CDK 可恢复当前会话；时效到期后 CDK 自动失效，未收到短信的 CDK 可再次使用。validate 接口有频率保护，Admin 登录有失败封禁，Admin 可在安全日志页查看异常登录记录。

**风险提示**
`expired` 路径需区分"未激活到期"和"已激活到期"两种情况，状态更新逻辑需确保 CDK 状态与订单状态同步写入，避免 CDK 错误归还 active 或错误标为 exhausted。`login_attempts` 表会持续增长，上线后需定期清理（保留最近 30 天即可）。
...
| 9    | 中       | expired 路径需区分两种情况，CDK 状态与订单状态需同步写入；validate 改动涉及 count 和 timed 两种类型，需回归测试；login_attempts 表需定期清理 |
